...
void do_exit(struct Shell *shell, int argc, const char *args[argc]) {
    shell->stop = true;
}

void do_cd(struct Shell *shell, int argc, const char *args[argc]) {
    if (argc == 1) {
        
                // go HOME
        } else {
            // use first arg, ignore others
        }   
}
...
void shell_print_prompt(struct Shell *shell)
{
    printf("%d> ", shell->nb_commands);
}
